...
class Create_random_polygon:
    ...
    def generate_random_points(self):
        random_coords_list = []
        min_distance = (self.max_rand_coord-self.min_rand_coord)/10
        for x in range(self.points_num):
            ...
            while too_close:
                ...
                if random_coords_list == []:
                    break
                else:
                    for other_coords in random_coords_list:
                        distance = numpy.sqrt((coords_tuple[0]-other_coords[0])**2+(coords_tuple[1]-other_coords[1])**2)
    ...
    def main(self, plot = False):
        ...
        if self.array == None:
            if not all(
                [isinstance(self.min_rand_coord, int),
                 isinstance(self.max_rand_coord, int),
                 isinstance(self.points_num, int),]
                ):
                print('Error! Values must be "integer" type:', 'min_rand_coord =',self.min_rand_coord, ', max_rand_coord =',self.max_rand_coord, ', points_num =',self.points_num)
            else:                  
...
def gen_polygon(min_rand_coord,max_rand_coord,points_num):
    array = None
    crt = Create_random_polygon(array, min_rand_coord, max_rand_coord, points_num)
    return crt.main(plot = False)
